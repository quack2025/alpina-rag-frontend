...
const AdvancedPersonaManager: React.FC<AdvancedPersonaManagerProps> = ({ 
  ...
  const [generationConfig, setGenerationConfig] = useState({
    archetype: AlpinaArchetype.PROFESIONAL as AlpinaArchetype,
    count: 10,
  ...
  const archetypeDescriptions = {
    ...
    [AlpinaArchetype.GOMOSO_EXPLORADOR]: {
  ...
  const loadPersonas = () => {
    const saved = localStorage.getItem('alpina_advanced_personas');
    if (saved) {
  ...
  const savePersonas = (newPersonas: SyntheticPersona[]) => {
    ...
    localStorage.setItem('alpina_advanced_personas', JSON.stringify(newPersonas));
  };

  // Generar persona base desde arquetipo
  const generatePersonaFromArchetype = (archetype: AlpinaArchetype, variant?: any): SyntheticPersona => {
    ...
    const persona: SyntheticPersona = {
      ...
      metadata: {
        ...
        created_by: 'system',
        version: 1,
        tags: [archetype, 'honduras', 'alpina']
      }
    };
    ...
    for (let i = 0; i < generationConfig.count; i++) {
      ...
      if (generationConfig.includeCounterStereotypical && Math.random() < generationConfig.counterStereotypicalPercentage / 100) {
        ...
        persona.characteristics.psychographics.innovation_adoption = 
          persona.archetype === AlpinaArchetype.RESIGNADO ? 'early_adopter' : 'laggard';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900">
                Sistema Avanzado de Personas Sintéticas - Alpina Honduras
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(archetypeDescriptions).map(([key, archetype]) => (
                <div key={key} className="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition-colors">
                  ...
                  <button
                    onClick={() => {
                      setGenerationConfig(prev => ({ ...prev, archetype: key as AlpinaArchetype }));
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {Object.values(AlpinaArchetype).map(archetype => {

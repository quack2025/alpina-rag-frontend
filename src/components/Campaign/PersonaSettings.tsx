...
import { AlpinaArchetype, ARCHETYPE_TEMPLATES } from '../../types/persona.types';
...
const PersonaSettings: React.FC<PersonaSettingsProps> = ({ 
  ...
  const loadPersonas = () => {
    const saved = localStorage.getItem('alpina_campaign_personas');
    if (saved) {
  ...
  const createDefaultPersonas = () => {
    ...
    Object.values(AlpinaArchetype).forEach(archetype => {
  ...
  const getDefaultName = (archetype: string): string => {
    const names = {
      [AlpinaArchetype.PROFESIONAL]: 'Carlos Eduardo Martínez',
      [AlpinaArchetype.CONTROLADOR]: 'María Elena Rodríguez',
      [AlpinaArchetype.EMPRENDEDOR]: 'José Antonio Mejía',
      [AlpinaArchetype.GOMOSO_EXPLORADOR]: 'Andrea Sofia Castillo',
      [AlpinaArchetype.PRAGMATICO]: 'Luis Fernando Paz',
      [AlpinaArchetype.RESIGNADO]: 'Pedro José Martínez'
    };
  ...
  const getDefaultLocation = (archetype: string): any => {
    const locations = {
      [AlpinaArchetype.PROFESIONAL]: { city: 'Tegucigalpa', department: 'Francisco Morazán', neighborhood: 'Col. Lomas del Guijarro' },
      [AlpinaArchetype.CONTROLADOR]: { city: 'San Pedro Sula', department: 'Cortés', neighborhood: 'Col. Jardines del Valle' },
      [AlpinaArchetype.EMPRENDEDOR]: { city: 'Choloma', department: 'Cortés', neighborhood: 'Barrio El Centro' },
      [AlpinaArchetype.GOMOSO_EXPLORADOR]: { city: 'Tegucigalpa', department: 'Francisco Morazán', neighborhood: 'Col. Palmira' },
      [AlpinaArchetype.PRAGMATICO]: { city: 'La Ceiba', department: 'Atlántida', neighborhood: 'Barrio La Isla' },
      [AlpinaArchetype.RESIGNADO]: { city: 'Juticalpa', department: 'Olancho', neighborhood: 'Barrio El Centro' }
    };
  ...
  const savePersonas = (personasToSave: SyntheticPersona[]) => {
    localStorage.setItem('alpina_campaign_personas', JSON.stringify(personasToSave));
    setHasChanges(false);
  };
